...
const (
	repoURL    = "https://api.github.com/repos/grafana/k6/releases/latest"
	installDir = "/usr/local/bin/k6"
)
...
func main() {
	...
	fmt.Println("🔍 Buscando a versão mais recente do k6...")
	...
	fmt.Println("✨ k6 instalado com sucesso em", installDir)
	exec.Command("k6", "version").Stdout = os.Stdout
}
...
func extractAndInstall(fileName string) error {
	...
	cmdMv := exec.Command("sudo", "mv", "k6", installDir)
